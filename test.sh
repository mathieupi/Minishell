...
cd $HOME
cd ' // '; pwd
./minishell -c "echo bonjour > file ; <file"
touch test ; < test
cd '' 
cd '' ''
cd '' '' ''
...
cat test.sh | grep arcu |  
echo test |   |   |   |   |   |   |   |   |   |  |  
cat /dev/random | head -c 100 | wc -c
echo testing multi ; echo "test 1 ; | and 2" ; cat test.sh | grep echo
>lol echo > test>lol>test>>lol>test mdr >lol test >test; cat test
touch a b c d; mkdir m1 m2 m3; echo bonjour > a ; ls -l |   < a
ls asdfasdf | echo a
echo a | ls asdfasdf

#Boucle infinie :
  /dev/random | head -c 10
  /dev/random |   | head -c 10maK
  /dev/random |   |   | head -c 10
